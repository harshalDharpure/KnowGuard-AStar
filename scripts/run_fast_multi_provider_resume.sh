#!/usr/bin/env bash
# Fast resume: parallel providers with separate quotas.
#   1× NIM Ultra (quality) + up to 2× OpenRouter Super:free + 1× Gemini lite
# Same JSONL paths → resume-safe. Mixed models → note in writeup.
set -euo pipefail
...
# Keep OpenRouter for :free models; do not unset it.
unset OPENAI_API_KEY
export KNOWGUARD_NIM_MIN_INTERVAL="${KNOWGUARD_NIM_MIN_INTERVAL:-2.0}"

PY="$ROOT/.venv/bin/python"
TAG="astar_lite_nim"
LOG="logs/fast_multi_provider_resume.log"
mkdir -p logs results
echo "[$(date)] fast multi-provider start" | tee -a "$LOG"

NIM_MODEL="nvidia/nemotron-3-ultra-550b-a55b"
OR_MODEL="nvidia/nemotron-3-super-120b-a12b:free"
GEM_MODEL="${GEMINI_EVAL_MODEL:-gemini-flash-lite-latest}"
...
count_provider() {
  # $1 = nvidia|openrouter|google
  pgrep -af "Open_benchmark.py --dev_filename shards/ioMEDQA_shard" 2>/dev/null \
    | grep -v grep | grep -c -- "--use_api $1" || true
}

start_shard() {
  local i=$1 provider=$2 model=$3
...
[[ -n "${NVIDIA_API_KEY:-}${NGC_API_KEY:-}" ]] || { echo "no NVIDIA key"; exit 1; }
...
while true; do
  ...
  n_nim=$(count_provider nvidia)
  n_or=$(count_provider openrouter)
  n_gem=$(count_provider google)

  # Fill NIM slot first (quality on densest)
  if [[ "$n_nim" -lt "$MAX_NIM" ]]; then
    for i in "${unfinished[@]}"; do
      if ! shard_running "$i"; then
        start_shard "$i" nvidia "$NIM_MODEL"
        n_nim=$((n_nim+1))
        break
      fi
    done
  ...
  # Fill OpenRouter free slots (speed)
  while [[ "$n_or" -lt "$MAX_OR" ]]; do
    started=0
    for i in "${unfinished[@]}"; do
      if ! shard_running "$i"; then
        start_shard "$i" openrouter "$OR_MODEL"
        n_or=$((n_or+1))
        started=1
        break
      fi
  ...
  # Gemini lite if key present and slot free
  if [[ -n "${GOOGLE_API_KEY:-}${GEMINI_API_KEY:-}" && "$n_gem" -lt "$MAX_GEM" ]]; then
    for i in "${unfinished[@]}"; do
      if ! shard_running "$i"; then
        start_shard "$i" google "$GEM_MODEL"
        break
      fi
    done
  fi
