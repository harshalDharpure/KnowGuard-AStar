...
TAG="${TAG:-astar_nim}"
CAMPAIGN_LOG="logs/astar_campaign.log"
...
"
  ...
  echo "[$(date)] === START split N=$N dev=$DEV ===" | tee -a "$CAMPAIGN_LOG"
  ...
  echo "[$(date)] === DONE N=$N ===" | tee -a "$CAMPAIGN_LOG"
  cat "$MET" | tee -a "$CAMPAIGN_LOG"
}

# Run all splits sequentially
for N in 20 50 1272; do
  run_split "$N"
done

echo "[$(date)] === CAMPAIGN COMPLETE ===" | tee -a "$CAMPAIGN_LOG"
